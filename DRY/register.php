...
if(isset($_POST['userName'])) {
    if (existsUser($_POST['userName'])){
        ...
        $userInfo[$_POST['userName']]['password'] = sha1($_POST['inputPassword']);
        $userInfo[$_POST['userName']]['birthday'] = $_POST['userBirthday'];
        setUsersList($userInfo);
        session_start();//старт сессии
        $_SESSION['auth'] = true; //авторизовались
        $_SESSION['userName'] = $_POST['userName'];
        header("Location: /");// ссылка на страницу, которая откроется после регистрации
    }
}
?>
